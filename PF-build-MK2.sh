...
#                          So did I
# 13 Jun 2019, 3d-gussner, Added compiling of MK2 branch
...
# Languages are build in MK2 branch differently than in MK3 branch 
...
#Check if DEV_STATUS is selected via argument 2
if [ ! -z "$2" ] ; then
	if [[ "$2" == "GOLD" || "$2" == "RC" || "$2" == "BETA" || "$2" == "ALPHA" || "$2" == "DEVEL" || "$2" == "DEBUG" ]] ; then
		DEV_STATUS_SELECTED=$2
	else
...
do
	...
	if [ -f "$SCRIPT_PATH/../$OUTPUT_FOLDER/FW$FW-Build$BUILD-$VARIANT.hex" ]; then
		echo ""
...
# MK2 branch languages are build before compiling the firmware
#	if [ $LANGUAGES ==  "ALL" ]; then
#		echo "$(tput setaf 2)"
...
#	fi
	cp -f firmware.ino.hex $SCRIPT_PATH/../$OUTPUT_FOLDER/FW$FW-Build$BUILD-$VARIANT.hex
